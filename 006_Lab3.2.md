...
### Step 2 – Define the Document State Structure
...
### Step 3 – File Upload and Text Extraction
...
**Explanation – `upload_document()`**

* Prints an instruction: *“Please upload a document (PDF, DOCX, TXT, or XLSX):”*
...
**Explanation – `extract_text_from_file(file_name, file_content)`**
...
**Explanation – `process_uploaded_document()`**
...
### Step 4 – Validation, Cleaning, Summarization, Keyword Extraction
...
**Explanation – `validate_input(state)`**

* Checks:

  * If there is any `raw_document` content.
  * If length ≥ 10 characters (otherwise error: “Document too short…”).
  * If length ≤ 50,000 characters (otherwise error describing how large it is).
...
**Explanation – `clean_document(state)`**
...
**Explanation – `summarize_document(state)`**
...
**Explanation – `extract_keywords(state)`**

* Uses `ChatOpenAI` again with lower temperature (0.2).
* Builds a prompt using the summary (not the raw text) and asks for a **comma-separated list** of 5–7 keywords.
* Parses the response:
...
### Step 5 – Routing, Error Handler, and Success Handler
...
* Flow:
  ...
  * `"clean"` → `"summarize"` → `"extract_keywords"` → `"success_handler"` → `END`.
  * `"error_handler"` → `END`.
* `graph = builder.compile()` makes an executable pipeline from this definition.

---

### Step 7 – Main Execution: Upload and Process a Document
...
* Prints a header: *“Document Processing Pipeline”*.
...
### Step 8 – Alternative: Process a Built-In Sample Document
...
### Step 9 – Usage Options / How to Run in Class
...
* This lets you demo both flows:
  ...
  * **Sample mode** – deterministic, good for projector demo.
  * **Upload mode** – interactive, lets participants bring their own PDFs, DOCX, or Excel reports.
